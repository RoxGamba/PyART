...
parser.add_argument(
    "-i", "--id", default=1, type=int, help="Simulatoion ID (-1 to show all captures)"
)
parser.add_argument("--f0", default=0.002, type=float, help="FFI f0")
parser.add_argument(
    "--integration_test",
    action="store_true",
    help="Show integration test for new integrator",
)
...
for ID in IDs:
    ...
    if args.integration_test:
        ...
        mode_old = Multipole(
            l, m, t, signal, mass=M, radius=icc.r_extr, integrand=integrand
        )
    ...
    iicc = Waveform_ICC(
        path=path, ID=ID, integrate=True, integr_opts=integr_opts, load_puncts=True
    )
