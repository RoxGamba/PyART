...
class Spanner(object):
    def __init__(
        ...
        if self.input_file is None:
            ...
            self.run_TEOB_parallel(
                nproc=nproc, dump_npz=dump_npz, ignore_input_file=ignore_input_file
            )
        else:
            if self.verbose:
            ...
            if not np.allclose(
                data["meta"],
                [self.q, self.chi1, self.chi2, self.r0, self.nj],
                atol=1e-12,
            ):
                print(
    ...
    def parspace_populate(self):
        ...
        if self.update_pph_min:
            ...
            for j in pph_sample_tmp:
                E_max = EnergyLimitsSpin(10.0, self.q, j, self.chi1, self.chi2, N=1000)
                if E_max >= 1:
                    self.pph_min = j
                    break
        ...
        for i, pph in enumerate(pph_sample):
            E_max_pph[i] = max(
                1, EnergyLimitsSpin(10.0, self.q, pph, self.chi1, self.chi2, N=1000)
            )
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            for i in range(nproc):
                ...
                task = executor.submit(
                    run_TEOB_list, batches[i], self.points, self.eobcommonpars
                )
                tasks.append(task)
    ...
    def get_NR_points(
        ...
        nu = self.nu
        if Emax is None:
            Emax = max(self.points[:, 1])
        if Jmax is None:
            Jmax = max(self.points[:, 0]) * nu
        if Jmin is None:
            Jmin = min(self.points[:, 0]) * nu

        data_NR = []
        NR_path = os.path.join(repo_path, "data", dset.lower())
        sims = simulations.Sim(dset=dset)
        NR_list = sims.subset(
            pars={"q": q, "chi1z": self.chi1, "chi2z": self.chi2}, float_tol=5e-3
        )
        sims_to_skip = [
            "bbh_q-3.0_E0-1.017_j0-4.350_old-TP_N-96",
            "bbh_q-3.0_E0-1.017_j0-4.350_old-TP_N-128",
            "bbh_q-3.0_E0-1.017_j0-4.350_old-TP_N-192",
            "bbh_q-3.0_E0-1.017_j0-4.350_old-TP_N-256",
            "bbh_q-3.0_E0-1.017_j0-4.400_gauge-TP_N-128",
            "bbh_q-3.0_E0-1.017_j0-4.400_gauge-TP_N-192",
            "bbh_q-3.0_E0-1.017_j0-4.400_gauge-TP_N-256",
            "bbh_q-1.0_E0-1.011_j0-3.950_testM2_N-96",
        ]
        for name in NR_list:
            ...
            psi4_l2m2 = np.loadtxt(
                os.path.join(NR_path, name, "collated_smalldata/psi4_l2m2_r100.00.txt")
            )
            r_end = (
                np.sqrt(
                    (punct0[-1, 2] - punct1[-1, 2]) ** 2
                    + (punct0[-1, 3] - punct1[-1, 3]) ** 2
                )
                / M
            )
            ...
            t = psi4_l2m2[:, 0] / M
            dT = t[1] - t[0]
            psi4_peaks, _ = find_peaks(
                psi4_amp, height=0.0002, distance=round(time_distance / dT)
            )
    ...
    def plot_parspace(
        ...
        write_text(
            0.01, 0.97, r"$(q,\chi_1,\chi_2)=(" + self.qchi1chi2_list_str() + ")$"
        )
        write_text(0.10, 0.65, r"$E_0>V_0^{\rm max}$")
        write_text(0.70, 0.37, r"scattering", clr=[1, 1, 1, 1])

        if show_NR:
            NR_points = self.get_NR_points(
                dset=dset,
                Emin=Emin,
                Emax=Emax,
                Jmin=pph_min * self.nu,
                Jmax=pph_max * self.nu,
            )
            plt.scatter(
                NR_points["J0_mrg_single"],
                NR_points["E0_mrg_single"],
                color=[0, 1, 0],
                s=5,
                marker="o",
            ...
            plt.scatter(
                NR_points["J0_mrg_multi"],
                NR_points["E0_mrg_multi"],
                color=[1, 0.8, 0],
                s=5,
                marker="o",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--show_NR", action="store_true", help="Show NR points on parspace plot (bool)"
    )
    parser.add_argument(
        "--dset",
        choices=["GAUSS_2021", "GAUSS_2023"],
        default="GAUSS_2023",
        type=str,
        help="dset to use for show_NR",
    )
    parser.add_argument(
