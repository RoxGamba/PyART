...
from PyART.models.teob import (
    search_radial_turning_points,
    RadialPotential,
    SpinHamiltonian,
)
...
class Spanner(object):
    ...
    def __init__(
        ...
        if r0_type != "auto" and r0_type != "val":
            raise ValueError(
                f"Unknown option for r0_type: '{r0_type}'. Use 'auto' or 'val'"
            )
        ...
        self.eobcommonpars = {
            ...
            "srate_interp": 300000.0,  # srate at which to interpolate. Default = 4096.
        ...
        if self.input_file is None:
            ...
            for key, val in meta.items():
                ...
                if isinstance(val, float) and isinstance(self_attr, float):
                    meta_are_equal = meta_are_equal and np.isclose(
                        self_attr, val, atol=1e-12
                    )
    ...
    def determine_r0_from_pphE(
        self, pph, E, apa_tol=None, rVmin=None, debug_plot=False
    ):
        if E >= 1:  # unbound case
            if self.r0_type == "auto":
                r0 = self.r_infty
            ...
            _, r_apa = search_radial_turning_points(
                self.q, self.chi1, self.chi2, pph, E, step_size=0.1
            )
            if self.r0_type == "auto":
                r0 = r_apa
                if debug_plot:
            ...
            if rVmin is None:
                rVmin = 0.0  # fictitious value
            if r0 is not None and r0 < rVmin:
                if self.vverbose:
                    print(
                        f"   discarding pph0, E0, r0 : {pph:8.5f} {E:8.5f} {r0:8.3f}  (rVmin :{rVmin:7.3f})"
                    )
                r0 = None
            elif r0 is None and self.vverbose:
                print(
                    f"   discarding pph0, E0, r0 : {pph:8.5f} {E:8.5f}   nan     (rVmin :{rVmin:7.3f})"
                )
        return r0
    ...
    def parspace_populate(self):
        ...
        if self.verbose:
            print(
                "   pph such that Vmax~1: {:.6f} (Vmax={:.6f})".format(
                    pph_Vmax_approx1, E_max_j
                )
            )
        ...
        if self.Emax > 1.0:
            ...
            for i, pph in enumerate(pph_sample_unbound):
                E_max_pph[i] = max(
                    1, EnergyLimitsSpin(rmax, pph, self.q, self.chi1, self.chi2, N=1000)
                )
            for i in range(nj):
                for j in range(nj):
                    if j <= i:
            ...
            if self.verbose:
                print(
                    f"   considering {npoints_unbound:5d} configurations for unbound motion"
                )
        else:
            npoints_unbound = 0
        ...
        if Emin < 1.0:
            ...
            dj = (
                pph_sample_unbound[2] - pph_sample_unbound[1]
            )  # dj used in the unbound case
            num_new_pph = int(
                (pph_Vmax_approx1 - self.pph_min) / dj
            )  # new (lower) pph to consider in the bound case
            new_pph = np.linspace(self.pph_min, pph_Vmax_approx1, num=num_new_pph)
            pph_sample = np.unique(
                np.concatenate((new_pph, pph_sample_unbound))
            )  # concatenate
            ...
            for i in range(nE):
                for j in range(npph):
                    E0 = bounded_E[i]
                    pph0 = pph_sample[j]
                    Vmax, Vmin, rVmax, rVmin = RadialPotential_MaxMin(
                        100, pph0, self.q, self.chi1, self.chi2, N=1000
                    )
                    if Vmax is None or Vmin is None:
                        continue
                    if Vmax >= E0 and Vmin <= E0:
                        info_to_compute_r0.append(
                            [pph0, E0, rVmin, 0.0]
                        )  # last column will be filled with r0
                    elif E0 < Vmin:
                        break
            ...
            if self.verbose:
                print(
                    "   computing/checking r0 for {:d} bounded configurations - {:d} core(s)".format(
                        len(info_to_compute_r0), self.nproc
                    )
                )
            Y = self.__parallel_run(
                self.compute_r0_from_IClist, info_to_compute_r0, apa_tol
            )
        ...
        if self.verbose:
            print(
                "   points: {:d} (unbound: {:d}, bound: {:d})".format(
                    self.npoints, npoints_unbound, npoints_bound
                )
            )
    ...
    def single_run_TEOB(self, point):
        ...
        while Eend < 1 and rend > 3:
            if counter == maxiter:
                if self.verbose:
                    print(
                        f"   Warning: (E0,pph0)=({point[1]:.5f}, {point[0]:.5f}) - ",
                        end="",
                    )
                    print(
                        f"(Eend,rend)=({Eend:.10f}, {rend:.1e}) - maxiter ({maxiter}) reached, strategy: {strategy}"
                    )
                    if strategy == "skip":
                        dyn["E"] = (
                            0 * dyn["E"]
                        )  # with this value set npeaks and rend to NaN, see next if-statement
                    elif strategy == "ignore":
                        pass  # do nothing
                    else:
                        raise RuntimeError(
                            f"Unknown strategy: '{strategy}' - strategy should be 'skip' or 'ignore'"
                        )
                break
        ...
        if Energy[-1] < 1e-5 or Energy[-1] > point[1]:
            if self.vverbose:
                print(
                    "   TEOB failure at pph0, E0, r0 : {:8.5f} {:8.5f} {:7.3f}, final En: {:9.5f}".format(
                        point[0], point[1], point[2], Energy[-1]
                    )
                )
            npeaks = np.nan
            rend = np.nan
    ...
    def run_TEOB_parallel(
        self, dump_npz=False, dump_txt=False, ignore_input_file=False
    ):
        if self.input_file is not None and not ignore_input_file:
            if self.verbose:
                print(
                    ">> Provided input file and ignore_input_file is False, skipping TEOB-runs"
                )
            return
        ...
        t0 = time.perf_counter()
        self.points = self.__parallel_run(
            self.run_TEOB_list, self.points, self.eobcommonpars
        )
    ...
    def plot_parspace(
        ...
        plt.scatter(
            points2plot[~mask_nan, 0],
            points2plot[~mask_nan, 1],
            c=N[~mask_nan],
            cmap=cmp,
            s=marker_size,
        )  # , zorder=3)
        ...
        write_text(
            0.02, 0.97, r"$(q,\chi_1,\chi_2)=(" + self.qchi1chi2_list_str() + ")$"
        )
        ...
        if qc_line and Emin < 1:
            ...
            for i in range(npph):
                qc_pph_i = qc_pph[i]
                _, Vmin, _, rVmin = RadialPotential_MaxMin(
                    rmax_search, qc_pph_i, self.q, self.chi1, self.chi2, N=1000
                )
                if Vmin > Emin:
                    qc_E[i] = Vmin
                else:
        ...
        if show_kankani:
            if not np.allclose([self.q, self.chi1, self.chi2], [1, 0, 0]):
                print(
                    "Warning: Kankani-McWilliams fit works only for nonspinning equal mass. Skipping..."
                )
            else:
                ca = 3.8733
                cb = -6.7554
        ...
        if show_gra_fit:
            if not np.allclose([self.q, self.chi1, self.chi2], [1, 0, 0]):
                print(
                    "Warning: our fit works only for nonspinning equal mass. Skipping..."
                )
            else:
                ca = 15.3129
                cb = -26.5987
        ...
        if grey_fill:
            ...
            mask = triangle_path.contains_points(
                np.vstack((X.flatten(), Y.flatten())).T
            )
            mask = mask.reshape(X.shape)

            plt.scatter(
                X[~mask], Y[~mask], color=[0.9, 0.9, 0.9], s=1, zorder=1, alpha=0.05
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Emin", default=1.0, type=float, help="minimum energy to consider"
    )
    parser.add_argument(
        "--Emax",
        default=2.0,
    ...
    parser.add_argument(
        "--pph_max", default=5.0, type=float, help="max value of pph considered"
    )
    parser.add_argument(
        "--Nmax_plot", default=None, type=int, help="N-max of peaks to show in plots"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-vv", "--vverbose", action="store_true", help="very verbose option"
    )
    parser.add_argument(
        "--savepng", action="store_true", help="save parspace-plot as png"
    )
    parser.add_argument(
        "--figname", default=None, type=str, help="name of the saved figure (png)"
    )
    parser.add_argument(
        "--show_fails", action="store_true", help="Show failed runs in parspace plot"
    )
    parser.add_argument(
        "--continuous_cmap",
        action="store_true",
    ...
    parser.add_argument(
        "--dump_txt", action="store_true", help="dump the points used in txt file"
    )
    parser.add_argument(
        "--show_kankani",
        action="store_true",
    ...
    parser.add_argument(
        "--show_gra_fit", action="store_true", help="Show transition fit from GRA data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--parabolic_line", action="store_true", help="show line for the parabolic limi"
    )
    parser.add_argument(
        "--qc_line", action="store_true", help="show QC line in parspace-plot if Emin<1"
    )
    parser.add_argument(
        "--grey_fill", action="store_true", help="fill with grey the E>V region"
    )
    parser.add_argument(
        "--show",
        choices=["on", "off"],
