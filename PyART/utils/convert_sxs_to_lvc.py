...
# RG
# Taken from https://github.com/sxs-collaboration/catalog_tools/tree/master,
# with very slight modifications to not need the sxs_catalog.json and the
# sxs_catalog_resolutions.json files.
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--truncation_time",
        help="If specified, truncate time series at this time instead of at the reference time",
        type=float,
        default=None,
    )
    args = p.parse_args()
...
def waveform_norm_squared(sxs_format_waveform, extrapolation_order="Extrapolated_N2"):
    ...
    for key in sxs_format_waveform[extrap].keys():
        if "Y_" in key:
            for i in range(1, 3):
                sum_amp_squared += np.square(sxs_format_waveform[extrap][key][:, i])
    return sum_amp_squared


def peak_time_from_sxs(
    sxs_format_waveform, metadata, extrapolation_order="Extrapolated_N2"
):
    ...
    # times
    times = sxs_format_waveform[extrapolation_order + ".dir"]["Y_l2_m2.dat"][:, 0]
...
def spline_amp_phase_from_sxs(
    ...
    inclusive."""
    modes, times, amps, phases, start_time, peak_time, l_max = amp_phase_from_sxs(
        sxs_format_waveform, metadata, modes, extrapolation_order, truncation_time
    )
    spline_amps = []
    spline_phases = []
...
def write_splines_to_H5(out_filename, modes, spline_amps, spline_phases, times):
...
def derived_horizon_quantities_from_sxs(sxs_horizons, start_time, peak_time):
    ...
    t_A, x_A = prepare_horizon_quantity(
        sxs_horizons["AhA.dir"]["CoordCenterInertial.dat"], start_time, peak_time
    )
    t_B, x_B = prepare_horizon_quantity(
        sxs_horizons["AhB.dir"]["CoordCenterInertial.dat"], start_time, peak_time
    )
    # This is used only for t_C
    t_C, x_C = prepare_horizon_quantity(
        sxs_horizons["AhC.dir"]["CoordCenterInertial.dat"], start_time, peak_time
    )
    ...
    # of points.
    r_cross_v = np.array(
        [np.cross(n_vec[i], dn_vec_dt[i]) for i in range(len(dn_vec_dt))]
    )
...
def insert_derived_spline(spline_dictionary, spline_keys, derived_quantity):
    ...
    for a in range(0, len(derived_quantity[0]) - 1):
        spline_AH_list.append(romspline.ReducedOrderSpline(times_AH, quantity_AH[:, a]))
...
def horizon_splines_from_sxs(horizons, start_time, peak_time):
    ...
    n_hat, omega_orbit, LN_hat, t_A, t_B, t_C = derived_horizon_quantities_from_sxs(
        horizons, start_time, peak_time
    )
    insert_derived_spline(
        horizon_splines, ["nhatx-vs-time", "nhaty-vs-time", "nhatz-vs-time"], n_hat
    )
    insert_derived_spline(horizon_splines, ["Omega-vs-time"], omega_orbit)
    insert_derived_spline(
        horizon_splines, ["LNhatx-vs-time", "LNhaty-vs-time", "LNhatz-vs-time"], LN_hat
    )
...
def simulation_type_from_spins(dimensionless_spin_1, dimensionless_spin_2):
    ...
    if (
        abs(dimensionless_spin_1[2]) > spin_zero_threshold
        or abs(dimensionless_spin_2[2]) > spin_zero_threshold
    ):
        simulation_type = aligned_type

    if (
        abs(dimensionless_spin_1[0]) > spin_zero_threshold
        or abs(dimensionless_spin_2[0]) > spin_zero_threshold
        or abs(dimensionless_spin_1[1]) > spin_zero_threshold
        or abs(dimensionless_spin_2[1]) > spin_zero_threshold
    ):
...
def write_metadata_from_sxs(
    out_filename, resolution, metadata, start_time, peak_time, l_max
):
    ...
    if str(eccentricity)[0] == ">":
        ...
        str(eccentricity) == "[simulation too short]"
    ...
    out_file.attrs["simulation-type"] = simulation_type_from_spins(
        dimensionless_spin1, dimensionless_spin2
    )
...
def convert_simulation(
    sxs_data_path, resolution, modes, out_path, truncation_time=None
):
    ...
    out_name = (
        out_path + "/" + sxs_id.replace(":", "_") + "_Res" + str(resolution) + ".h5"
    )
    log("Output filename is " + out_name)

    modes, times, spline_amps, spline_phases, start_time, peak_time, l_max = (
        spline_amp_phase_from_sxs(
            rhOverM, metadata, modes, extrapolation_order, truncation_time
        )
    )
    ...
    write_horizon_splines_from_sxs(out_name, horizon_splines_to_write, t_A, t_B, t_C)

    write_metadata_from_sxs(
        out_name, resolution, metadata, start_time, peak_time, l_max
    )
    ...
    with open(os.path.realpath(__file__), "r") as file:
        out_file["auxiliary-info"].create_dataset(
            "convert_sxs_to_lvc.py", data=file.read()
        )

    if "VersionHist.ver" in list(rhOverM.keys()):
        log("Writing VersionHist.ver")
        out_file["auxiliary-info"].create_dataset(
            "VersionHist.ver", data=rhOverM["VersionHist.ver"]
        )
    else:
        log("No VersionHist.ver found. Data being converted is version 0.")
    log("Writing log")
    out_file["auxiliary-info"].create_dataset("ConversionLog.txt", data=history)
    out_file.close()
...
if __name__ == "__main__":
    convert_simulation(
        args.sxs_data, args.resolution, modes, args.out_path, args.truncation_time
    )
