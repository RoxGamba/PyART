...
KEYS = [
    ...
    "S2",  # spins (3-vector)
    "chi1x",
    "chi2x",  # dimensionless spin x-components
    "chi1y",
    "chi2y",  # dimensionless spin y-components
    "chi1z",
    "chi2z",  # dimensionless spin z-components
    "LambdaAl2",
    "LambdaBl2",
    "r0",  # distance
    "e0",  # eccentricity
    ...
    "E0byM",  # E0/M
    "pos1",
    "pos2",  # position vectors
    "f0v",  # orbital freq (3-vector)
    "f0",  # orbital freq (float))
    "Mf",
    "af",  # remnant properties, scalars (af dimensionless, float)
    "afv",  # af (3-vector)
...
def errwarn(msg, raise_err=True):
    """
    Raise error or warning with given message
    ...
    raise_err : bool, optional
        If True, raise RuntimeError. If False, issue a warning. Default is True.
    """
    if raise_err:
...
def check_metadata(meta, raise_err=True):
    ...
    raise_err : bool, optional
        If True, raise error on problems. If False, issue warnings. Default is True.
    """
    # check that all required keys are in the dict
    for rkey in KEYS:
        if not rkey in meta:
            errwarn(
                f"Key '{rkey}' is required but not found in metadata!",
                raise_err=raise_err,
            )
    for key in meta:
        if not key in KEYS:
            errwarn(
                f"Unknown key '{key}'! If you think this is correct, updated cat_utils.py",
                raise_err=raise_err,
            )
    pass
...
def get_flags(meta, thres_spin=1e-5, thres_q=1e-3, thres_e0=1e-3):
    """
    Get flags for given metadata

    Parameters
    ----------
    meta : dict
        Metadata dictionary to analyze
    thres_spin : float, optional
        Threshold for spin to be considered zero (default is 1e-5)
    thres_q : float, optional
        Threshold for mass ratio to be considered equal mass (default is 1e-3)
    thres_e0 : float, optional
        Threshold for eccentricity to be considered quasi-circular (default is 1e-3)
    """
    flags = []
    spins = [
        meta["chi1x"],
