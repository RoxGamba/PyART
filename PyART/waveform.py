...
class Waveform(object):
    ...
    def find_max(
        ...
        if wave == "psi4lm":
            if self.t_psi4 is None:
                raise RuntimeError(
                    "psi4-time not defined, but needed to search psi4-max!"
                )
            t = self.t_psi4
        else:
            t = self.u
    ...
    def dynamics_from_hlm(self, modes, warning=False):
        ...
        if not self.dothlm:
            if warning:
                warnings.warn("Warning: dothlm not found, computing derivatives of hlm")
            for mode in modes:
                self.dothlm[mode] = {}
    ...
    def integrate_data(
        ...
        for i, lm in enumerate(modes):
            ...
            mode = IntegrateMultipole(l, m, t, z, **integr_opts, mass=M, radius=radius)
...
def waveform2energetics(h, doth, t, modes, mnegative=False):
    ...
    * mnegative    : if True, account for the factor 2 due to m<0 modes
    ...
    for k, (l, m) in enumerate(modes):
        ...
        # Angular momentum
        dictdyn["dotJz"][(l, m)] = (
            fact * m * np.imag(h[(l, m)]["z"] * np.conj(doth[(l, m)]["z"]))
        )
        ...
        dictdyn["dotJy"][(l, m)] = (
            ...
            * np.real(
                h[(l, m)]["z"]
                * (
                    wf_ut.mc_f(l, m) * np.conj(dothlm1)
                    - wf_ut.mc_f(l, -m) * np.conj(dothlm_1)
                )
            )
        )
        dictdyn["dotJx"][(l, m)] = (
        ...
        dothlm1 = doth[(l, m + 1)]["z"] if (l, m + 1) in doth else 0 * h[(l, m)]["z"]
        dothl_1m1 = (
            doth[(l - 1, m + 1)]["z"] if (l - 1, m + 1) in doth else 0 * h[(l, m)]["z"]
        )
        dothl1m1 = (
            doth[(l + 1, m + 1)]["z"] if (l + 1, m + 1) in doth else 0 * h[(l, m)]["z"]
        )
        ...
        dotPxiy = (
            ...
            * doth[(l, m)]["z"]
            * (
                wf_ut.mc_a(l, m) * np.conj(dothlm1)
                + wf_ut.mc_b(l, -m) * np.conj(dothl_1m1)
                - wf_ut.mc_b(l + 1, m + 1) * np.conj(dothl1m1)
            )
        )
        dictdyn["dotPy"][(l, m)] = np.imag(dotPxiy)
        dictdyn["dotPx"][(l, m)] = np.real(dotPxiy)
        dictdyn["dotPz"][(l, m)] = fact * np.imag(
            doth[(l, m)]["z"]
            * (
                wf_ut.mc_c(l, m) * np.conj(doth[(l, m)]["z"])
                + wf_ut.mc_d(l, m) * np.conj(dotl_1m)
                + wf_ut.mc_d(l + 1, m) * np.conj(dothl1m)
            )
        )

        dictdyn["dotP"][(l, m)] = np.sqrt(
            dictdyn["dotPx"][(l, m)] ** 2
            + dictdyn["dotPy"][(l, m)] ** 2
            + dictdyn["dotPz"][(l, m)] ** 2
        )
        ...
        for kk in kks:
            dotk = "dot" + kk
            this_mode = integrate.cumulative_trapezoid(
                dictdyn[dotk][(l, m)], t, initial=0
            )
...
class WaveIntegrated(Waveform):
    ...
    def __init__(
        ...
        if "deg" not in integr_opts:
            integr_opts["deg"] = 0
        if "poly_int" not in integr_opts:
            integr_opts["poly_int"] = None
        if "extrap_psi4" not in integr_opts:
            integr_opts["extrap_psi4"] = False
        if "window" not in integr_opts:
            integr_opts["window"] = None
