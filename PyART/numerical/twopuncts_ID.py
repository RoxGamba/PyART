...
class TwoPunctID(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.TP_pars = {
            ...
            "par_S_minus2": Smy,
            "par_S_minus3": Smz,
            "center_offset1": offsetx,
            "center_offset2": offsety,
            "center_offset3": offsetz,
            "npoints_A": self.npoints_A,
            "npoints_B": self.npoints_B,
            "npoints_phi": self.npoints_phi,
    ...
    def run_TP_from_parfile(self, parfile):
        ...
        TP_cmd = self.TP_exe + " " + parfile
        runcmd(TP_cmd, workdir=self.outdir, out=outfile)
        return outfile
    ...
    def run_TP_wrapper(self, P, parfile=None, verbose=None):
        ...
        if verbose:
            print(
                f"{outfile:10s} : P:{P:.10f}  E_ADM:{E_ADM:.10f}  M1:{M1:.5f}  M2:{M2:.5f}"
            )
        return E_ADM, M1, M2
    ...
    def fit_iter(
        ...
        if nproc_updated < nproc and verbose:
            print(
                f"Warning: using less processor ({nproc_updated:d}) than requested ({nproc:d})"
            )
        nproc = nproc_updated
        ...
        if poly_order is None:
            poly_order = min(max(npoints - 1, 1), 5)
        ncoeffs = poly_order + 1
        if ncoeffs > npoints:
            raise ValueError(
                f"Cannot perform {poly_order:d}-order polynomial fit with {npoints:d} points"
            )

        if verbose:
            print(
                f"Performing fit using {npoints:d} points with {poly_order:d}-order polynomial"
            )
        ...
        energies_fine = np.linspace(
            energies[0] - dE_approx, energies[-1] + dE_approx, num=1000
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-D", "--distance", default=100, type=float, help="Initial separation"
    )
    parser.add_argument(
        "-E", "--energy", default=1.01, type=float, help="Initial energy"
    )
    parser.add_argument(
        "-L",
        "--ang_momentum",
        default=1,
        type=float,
        help="Initial orbital angular momentum",
    )
    parser.add_argument("-o", "--outdir", default="./out", help="Outdir")
    parser.add_argument(
        "-n", "--nproc", default=1, type=int, help="number of process used"
    )
    parser.add_argument(
        "--res",
        default=[8, 8, 6],
        nargs="+",
        type=int,
        help="Resolution to use: [npoints_A,npoints_B,npoints_phi]",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help="Initial guess for the momentum modulo",
    )
    parser.add_argument(
        "--npoints",
        default=4,
        type=int,
        help="Points to consider when searching initial guess",
    )
    parser.add_argument(
        "--step", default=0.05, type=float, help="Initial relative momentum step"
    )
    parser.add_argument(
        "--resize", default=100, type=float, help="Resize factor used in the search"
    )
    parser.add_argument(
        "--tol", default=1e-10, type=float, help="Tolerance on the energy"
    )
    ...
    Px, Py, E_adm = tpi.fit_iterations(
        P0=args.init_guess,
        step_rel=args.step,
        tol=args.tol,
