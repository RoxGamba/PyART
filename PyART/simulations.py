...
KEYMAP = {
    ...
    "J0": [
        "J0",
        "J_adm",
        "initial-ADM-angular-momentum-z",
        "initial_ADM_angular_momentum",
        "initial-ADM-angular-momentumz",
    ],
...
class Simulations:
    ...
    def __init__(
        ...
        for f in self.files:
            ...
            if ext == ".txt" or ext == ".bbh":
                ...
                if verbose:
                    print(
                        "Skip file {}, do not know how to read data from {}".format(
                            f, ext
                        )
                    )
                continue
            if not d:
                continue  # dict is empty, do not store
            ...
            if icc_add_meta:
                ...
                d["M"] = float(d["initial-bh-puncture-adm-mass1"]) + float(
                    d["initial-bh-puncture-adm-mass2"]
                )
            if verbose:
    ...
    def search(self, key, val, bound=0.0):
        """
    ...
    def www_get_RIT(self, simulation):
        ...
        ctag_ = ctag.replace(":", "-")
        DAT = [
            "Metadata/{}-{}-{}_Metadata.txt".format(ctag, rtag, itag),
