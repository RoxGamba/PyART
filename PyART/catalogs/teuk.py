...
class Waveform_Teuk(Waveform):
    ...
    def _locate_parfile(self):
        """
        Method to locate the parfile in a teuk_*_mAll or output
        dir. Indeed, Teukode stores a parfile.par in the outdir,
        so we search in all the teuk_*_m? sub-dirs and current
        dir
        """
        subdirs = os.listdir(self.path)
        parfile = None
        candidate = os.path.join(self.path, "parfile.par")
            ...
            for sd in subdirs:
                if sd[:9] == "teuk_HH10" and not ".tar.gz" in sd:
                    attempt = os.path.join(
                        self.path, sd, "parfile.par"
                    )  # these should be generated by the exe
                    if os.path.exists(attempt):
                        parfile = attempt
                        break
        if parfile is None:
            raise FileNotFoundError("Parfile not found!")
        return parfile

    def load_metadata(self):
        if self.input_meta == "locate":
            parfile_located = self._locate_parfile()
            mtdt, a, mu = self.__load_metadata_from_parfile(parfile_located)
        elif self.input_meta is not None:
    ...
    def load_hlm(self):
        ...
        elif self.datafmt == "teuk_mAll" or self.datafmt == "teuk_mAll_py":
