...
class Waveform_CoRe(Waveform):
    ...
    def __init__(
        ...
        if os.path.exists(self.core_data_path) == False:
            if download:
                ...
                raise FileNotFoundError(
                    f"The path {self.core_data_path} does not exist."
                )
        ...
        if self.run is None:
            ...
            subdirs = os_utils.find_dirs_with_token(
                self.core_data_path, "R0"
            )  # assume less than 10 runs
        ...
        for ky in dset.keys():
            if "rh" in ky:
                ...
                self.hlm[(ell, emm)] = {
                    ...
                    "p": p_fre,
                    "z": z_fre,
                    "real": A_fre * np.cos(p_fre),
                    "imag": -A_fre * np.sin(p_fre),
                }
        self._u = tnew
        self._t = tnew
...
def radius_extrap_polynomial(ys, rs, K):
    ...
    for i in range(1, N):
        if len(ys[i]) != L:
            raise ValueError(
                "Inhomogenuous data set encountered! Check if all ys are sampled "
                * "on the same grid"
            )
