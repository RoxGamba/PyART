...
class Cataloger(object):
    ...
    def optimize_mismatches(
        self, optimizer_opts={}, verbose=None, ranges={"pph0": [1, 10]}, nproc=1
    ):
        ...
        if nproc == 1 or nsubset == 1:
            ...
            for i in range(nproc):
                ...
                # launch process
                process = Process(
                    target=self.process_with_redirect,
                    kwargs={
                        "process_id": i,
                        "nproc": nproc,
                        "opts": {
        ...
        for name in subset:
            self.data[name]["Optimizer"] = Optimizer(
                self.data[name]["Waveform"], **optimizer_opts
            )
        pass

    def __is_in_valid_range(
        self,
        name,
        ranges,
    ):
    ...
    def get_colors_for_subset(self, subset, cmap_var, cmap_name="jet"):
        ...
        out = {"colors": colors, "indices": indices, "cmap": cmap, "range": crange}
        return out
    ...
    def mm_vs_M(
        ...
        colors_dict = self.get_colors_for_subset(
            subset, cmap_var=cmap_var, cmap_name=cmap
        )
        ...
        for i, name in enumerate(subset):
            ...
            ax.plot(
                masses,
                mm_data["mismatches"][name]["mm_vs_M"],
                label=name,
                c=colors[cidx],
                lw=0.6,
            )
