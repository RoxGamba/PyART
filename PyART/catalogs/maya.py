...
class MAYA(Waveform):
    ...
    def __init__(self, basepath="/data/numrel/MAYA/", id=None, ell_emms="all") -> None:
    ...
    def compute_initial_data(self):
        ...
        try:
            chi1x = float(mtdt["twopunctures::par_s_plus[0]"])
            chi2x = float(mtdt["twopunctures::par_s_minus[0]"])
            chi1y = float(mtdt["twopunctures::par_s_plus[1]"])
            chi2y = float(mtdt["twopunctures::par_s_minus[1]"])
        except KeyError:
            chi1x = 0.0
            chi2x = 0.0
            chi1y = 0.0
            chi2y = 0.0
        chi1z = float(mtdt["twopunctures::par_s_plus[2]"])
        chi2z = float(mtdt["twopunctures::par_s_minus[2]"])
        chi1 = np.array([chi1x, chi1y, chi1z])
        chi2 = np.array([chi2x, chi2y, chi2z])

        # masses
        m1 = float(mtdt["TwoPunctures::target_M_plus"])
        m2 = float(mtdt["TwoPunctures::target_M_minus"])
        M = m1 + m2
        X1 = m1 / M
        X2 = m2 / M
        ...
        Pm = [
            float(mtdt["twopunctures::par_P_minus[" + str(i) + "]"]) for i in range(3)
        ]
