...
class TEOBRingdown(object):
    def __init__(self,
                 ...
                 modes       = [(2,2)],
                 use_fits    = True,
                 noncircular = False, kind = 'aligned-spins-equal-mass', databases=['RIT']
                 ) -> None:
        """
    ...
    def _intrinsic_to_rdpars(self, modes):
        ...
        for mode in modes:
            ell, emm = mode[0], mode[1]
            self._eval_fits(nu, X1, X2, X12, 
                            a0, a12, S_hat, S_bar, chiA[2], chiB[2], 
                            ell, emm)
    ...
    def _set_rd_params(self, modes):
        ...
        for mode in modes:
            ...
            c2p    = alph21
            c1p    = dOmg*(1. + c3p + c4p)/(c2p* (c3p + 2* c4p))
    ...
    def _rd_template(self, x, ell, emm):
        ...
        a1     = self.rd_params[mode]['a1'];       a2 = self.rd_params[mode]['a2']
        a3     = self.rd_params[mode]['a3'];       a4 = self.rd_params[mode]['a4']
        b1     = self.rd_params[mode]['p1'];       b2 = self.rd_params[mode]['p2']
        b3     = self.rd_params[mode]['p3'];       b4 = self.rd_params[mode]['p4']
        sigmar = self.rd_params[mode]['alpha1']
        sigmai = self.rd_params[mode]['omega1']

        amp    = ( a1 * np.tanh(a2*x +a3) + a4 )
        phase  = -b1*np.log((1. + b3*np.exp(-b2*x) + b4*np.exp(-2.*b2*x))/(1.+b3+b4));  

        A      = amp * np.exp(-sigmar*x); # amplitude 
        phase  = - (phase - sigmai*x);    # phase, minus sign in front by convention
    ...
    def _eval_fits(self, 
        ...
        self.rd_params[(ell, emm)]['Apk']      = rdf._amplitude_peak(nu, X12, S_hat, a12, S_bar, a0, omgpk, ell, emm)
        ...
        self.rd_params['Mbhf']     = rdf._JimenezFortezaRemnantMass(nu, X1, X2, chi1, chi2, 1.)
        return
    ...
    def _noncircular_corrections(self, modes, kind='non-spinning-equal-mass', databases= ['RIT', 'SXS', 'ET']):
        ...
        for mode in modes:
            ...
            for key in convert.keys():
                ...
                # compute correction
                res = ncf.eval_fit(gc_fits_param, fqs_dict, fqs_string, kind, databases=databases)
                if key in ['Mbhf', 'af']:
...
if __name__ == '__main__':
    ...
    # test
    params = {'q': 1.4, 'chiA': [0., 0., 0.], 'chiB': [0., 0., 0.]}
