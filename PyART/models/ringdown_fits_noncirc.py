...
def select_fitting_quantities(dataset_type, quantity_to_fit):
    ...
    if(dataset_type=='non-spinning'):
        ...
        fitting_quantities_strings_list = []
        for fx in fitting_quantities_base_list: fitting_quantities_strings_list.append('chieff-'+fx)
...
def read_fit_coefficients(quantity_to_fit, 
    ...
    path   = os.path.join(coeffs_dir, f'Fitting_coefficients_{dataset_type}_{catalogs_string}_{template_model}_{fit_dim}_{fitting_quantities_string}_{quantity_to_fit}.csv')
...
def eval_fit(    quantity_to_fit, 
    ...
    for key in list(fitting_qs_d.keys()):
        if key not in fitting_quantities_string: fitting_qs_d.pop(key, None)
    ...
    try:
        coeffs = read_fit_coefficients(quantity_to_fit, 
                                    fitting_quantities_string,
                                    template_model,
                                    fit_dim,
                                    databases,
...
if __name__ == '__main__':
    ...
    fitting_quantities_dict   = {'Heff_til': Hefft, 
    ...
    result    = eval_fit('Mf', fitting_quantities_dict, qnts, 'non-spinning-equal-mass',  databases=['RIT', 'SXS', 'ET'], verbose=True)
    # S=0 Q neq 1
    qnts      = select_fitting_quantities('non-spinning', 'Mf')[0]
    result2    = eval_fit('Mf', fitting_quantities_dict, qnts, 'non-spinning',  databases=['RIT', 'SXS', 'ET'], verbose=True)
    # S neq 0 Q=1
    qnts     = select_fitting_quantities('aligned-spins-equal-mass', 'Mf')[0]
    result3    = eval_fit('Mf', fitting_quantities_dict, qnts, 'aligned-spins-equal-mass',  databases=['RIT'], verbose=True)
    ...
    result    = eval_fit('af', fitting_quantities_dict, qnts, 'non-spinning-equal-mass',  databases=['RIT', 'SXS', 'ET'], verbose=True)
    # S=0 Q neq 1
    qnts      = select_fitting_quantities('non-spinning', 'af')[0]
    result2    = eval_fit('af', fitting_quantities_dict, qnts, 'non-spinning',  databases=['RIT', 'SXS', 'ET'], verbose=True)
    # S neq 0 Q=1
    qnts     = select_fitting_quantities('aligned-spins-equal-mass', 'af')[0]
    result3    = eval_fit('af', fitting_quantities_dict, qnts, 'aligned-spins-equal-mass',  databases=['RIT'], verbose=True)
    ...
    result    = eval_fit('omega_peak22', fitting_quantities_dict, qnts, 'non-spinning-equal-mass',  databases=['RIT', 'SXS', 'ET'], verbose=True)
    # S=0 Q neq 1
    qnts      = select_fitting_quantities('non-spinning', 'omega_peak22')[0]
    result2    = eval_fit('omega_peak22', fitting_quantities_dict, qnts, 'non-spinning',  databases=['RIT', 'SXS', 'ET', 'RWZ'], verbose=True)
    # S neq 0 Q=1
    qnts     = select_fitting_quantities('aligned-spins-equal-mass', 'omega_peak22')[0]
    result3    = eval_fit('omega_peak22', fitting_quantities_dict, qnts, 'aligned-spins-equal-mass',  databases=['RIT'], verbose=True)
    ...
    result    = eval_fit('A_peak22', fitting_quantities_dict, qnts, 'non-spinning-equal-mass',  databases=['RIT', 'SXS', 'ET'], verbose=True)
    # S=0 Q neq 1
    qnts      = select_fitting_quantities('non-spinning', 'A_peak22')[0]
    result2    = eval_fit('A_peak22', fitting_quantities_dict, qnts, 'non-spinning',  databases=['RIT', 'SXS', 'ET', 'RWZ'], verbose=True)
    # S neq 0 Q=1
    qnts     = select_fitting_quantities('aligned-spins-equal-mass', 'A_peak22')[0]
    result3    = eval_fit('A_peak22', fitting_quantities_dict, qnts, 'aligned-spins-equal-mass',  databases=['RIT'], verbose=True)
    plt.plot(bmrg, result, label='non-spinning-equal-mass')
    plt.plot(bmrg, result2, label='non-spinning')
    plt.plot(bmrg, result3, label='aligned-spins-equal-mass')
    plt.xlabel('b_massless_EOB')
